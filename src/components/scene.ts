...
import { IAssetsController } from "../controller/assets/assetsController";
...
import { IEntityBase, EntityBase } from "./entities/base/entityBase";
import { IInputTypePointer } from "../controller/input/types/inputTypePointer";
...
export class Scene extends EntityBase implements IScene {
    private readonly _assets: IAssetsController;
    ...
    public initialize(surface: ISurfaceController, input: IInputController): void {
        if (this._surface) {
            this._surface.removeEventListener(SurfaceController.surfaceChangeEvent, this.refresh);
        }
        this._input = input;
        this._surface = surface;

        this._surface.addEventListener(SurfaceController.surfaceChangeEvent, this.refresh);
        this.onInitialized();
    }

    public override destroy(): void {
        super.destroy();
        this.surface.removeEventListener(SurfaceController.surfaceChangeEvent, this.refresh);
    }
